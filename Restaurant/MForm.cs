...
namespace Restaurant
{
    public partial class MForm : Form
    {
        ...
        private void SetTextlabel()
        {
            if (menu.Count != 0)
            {
                labelCount.Text = menu.Count >= 6 ?
                    $"с {SwipeID + 1} по {SwipeID + 6} из {menu.Count} товаров" :
        ...
        private void Search()
        {
            menu.Clear();
            SwipeID = 0;
            menu = model.Menu.Where(x => x.Name.Contains(textBoxSearch.Text)).ToList();
            CreateTile();
        }
        ...
        private void Swipe(SwipeType swipeType)
        {
            ...
            if (swipeType == SwipeType.Right && SwipeID + 7 < menu.Count - 1)
            {
                SwipeID++;
                CreateTile();
            }
        ...
        private void labelNothing_Click_1(object sender, EventArgs e)
        {
